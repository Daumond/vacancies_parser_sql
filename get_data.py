...
class HeadHunterAPI:
    ...
    def get_vacancies(self, employer_id, page_count=1):
        ...
        while self.__params['page'] < page_count:
            print(
                f"обработка стр {self.__params['page'] + 1} для работодателя {self.employers[employer_id]['name']}",
                end=": ")
            try:
    ...
    def save_data_to_database(self, database_name: str):
        ...
        :return:
        """
        self.__cfg.update({'dbname': database_name})
        conn = psycopg2.connect(**self.__cfg)
